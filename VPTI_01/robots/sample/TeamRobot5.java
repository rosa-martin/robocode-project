...
public class TeamRobot5 extends AdvancedRobot implements Environment {
    ...
    public void evaluateFitness(ArrayList<Genome> population) {
        ...
        for(Genome gene : population){
            ...
            for(int i = 0; i < NEAT_Config.INPUTS; i++){
				...
				float inputs[] = {
					wasHitByBullet, wasHitByEnemy, hitWall, spotEnemy, hitEnemy, missEnemy, (float) this.getEnergy(), (float) this.getX(), (float) this.getY(),
					(float) this.getHeading(), (float) absoluteBearing, (float) this.getGunHeading(), (float) this.getGunHeat(), (float) this.getRadarHeading(),
					(float) this.getVelocity(), eSpottedX, eSpottedY, eBearing, eHeading, eVelocity, eAbsBearing, eDistance, eBulletBearing, eBulletHeading, eBulletPower, 
					eBulletVelocity, eHitX, eHitY, eEnergy, bulletHitPower, bulletHitVelocity, bulletHitHeading, (float) this.getOthers(), (float) this.getDistanceRemaining()
				};
